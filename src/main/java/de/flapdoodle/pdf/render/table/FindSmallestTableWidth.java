...
public class FindSmallestTableWidth implements MinimalTableWidth {

	private static final int MAX_LOOPS = 100;

	@Override
	public RenderedTableDimension of(TableRenderer renderer, Table table, float startingWidth) {
		var minInitialHeight = MinimalTableWidth.tableHeight(renderer, table, startingWidth);
...
//      println("width($loops) -> $testWidth")
			var height = MinimalTableWidth.tableHeight(renderer, table, testWidth);
		...
		TableRenderer.Result result = MinimalTableWidth.renderTable(renderer, table, endRange);

		return new RenderedTableDimension(
			endRange, result.tableHeight(),
